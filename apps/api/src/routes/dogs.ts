...
dogs.get("/next", async (c) => {
  ...
  return c.json(success({
    ...result,
    image_url: getImageUrl(result.image_key as string),
  }));
});
...
dogs.get("/:id", async (c) => {
  ...
  if (!result) {
    return c.json({ success: false, error: { code: "NOT_FOUND", message: "Dog not found" } }, 404);
  }
...
dogs.post("/:id/rate", zValidator("json", z.object({ value: z.number().min(0.5).max(5).multipleOf(0.5) })), async (c) => {
  const id = parseInt(c.req.param("id"));
  ...
  try {
    ...
    `).bind(id, value, anonId).run();

    return c.json(success({ rated: true }));
  } catch (e) {
    // Likely duplicate rating
    return c.json({ success: false, error: { code: "ALREADY_RATED", message: "Already rated this dog" } }, 400);
  }
});
...
dogs.post("/", zValidator("json", z.object({
  ...
  const result = await c.env.DB.prepare(`
    ...
    VALUES (?, ?, ?, ?, 'approved')
    RETURNING id
  `).bind(name || null, imageKey, breedId, anonId).first();

  return c.json(success({ id: result?.id }), 201);
});

// POST /api/upload-url - Get presigned upload URL
dogs.post("/upload-url", zValidator("json", z.object({
